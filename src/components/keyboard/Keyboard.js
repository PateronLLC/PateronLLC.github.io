...
export const Keyboard = ({
	...
	const onClick = (value) => {
		console.log(value);
		if (animating === true) console.log('Guess animating...')
		else if (value === 'Enter' && currSquare[1] >= squares[0].length) {
			onEnter();
	...
	useEffect(() => {
		const handleKeyUp = ({ key }) => {
			switch (true) {
				...
				default:
					console.error(
						"Key down event didn't match a letter, enter, or backspace."
					);
			}
		};
		...
		>
			...
			>
				<Key
					...
					value="Delete"
					onClick={onClick}
					colorKeyboardValue="rgb(226, 232, 240)"
				>
					Delete
				</Key>
				<Key
			...
			>
				{['Q', 'W', 'E', 'R', 'T', 'Y', 'U', 'I', 'O', 'P'].map((key) => (
					<Key
						...
						onClick={onClick}
						colorKeyboardValue={
							Object.prototype.hasOwnProperty.call(colorKeyboard, key)
								? colorKeyboard[key]
								: 'rgb(226, 232, 240)'
						}
					/>
