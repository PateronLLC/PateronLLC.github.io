...
function App() {
	...
	const resetBoard = () => {
		...
		setGameOver(false);
		new Promise((resolve) =>
			setTimeout(() => resolve('Reset state to new game'), 0)
		).then(() => {
		...
		if (!WORDLIST.includes(currGuess.toLowerCase()) || usedWord) {
			...
			squares[currSquare[0]].forEach((square, squareIndex) => {
				if (
					...
					finalWordLetterCache[square]--;
					if (newColorKeyboard[square] !== 'green')
	...
	function toggleModal(modalName) {
		...
		setModalState((prevState) => {
			return Object.assign(defaultModalState, {
				[modalName]: !prevState?.[modalName],
			});
		});
	}

	function handleUser(userId, username) {
  ...
  }, [modalState.loginModal, modalState.createUserModal, modalState.logoutModal])
